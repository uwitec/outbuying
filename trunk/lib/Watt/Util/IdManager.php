...
class Watt_Util_IdManager{
	
	private static $_managerPool = array();
	
	/**
	 * @return Watt_Util_IdManager
	 */
	public static function factory( $managerName = null ){
		...
		if( !key_exists( $managerName, self::$_managerPool ) ){
			self::$_managerPool[ $managerName ] = new Watt_Util_IdManager( $managerName );
		}
	...
	private function __construct( $managerName = null ){
		...
		$this->_db = Watt_Db::getDb();	// 原为getDbx() 导致 Watt_db->_connection 访问出错，因此更改为getDb()
	}
