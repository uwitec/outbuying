...
//				 */
//												
//				$hash_table[$cd_id][0] = Pft_I18n::trans( $menu->getCdMingcheng() );
...
//			}
			...
			if( Pft_Session::getSession()->isOutterUser()/*key_exists( $roleId, $crSessionRoleId )*/ ){
			...
			*/
			...
			if( (Pft_Session::getSession()->getSession()->getRoleShortname() == 'CR' || Pft_Session::getSession()->getSession()->getRoleShortname() == 'CRCPM') 
				&& Pft_Session::getSession()->getData('kh_zizhuxiadan') > 0
			){
				...
				if( Pft_Session::getSession()->isTq() ){
					...
					if(Pft_Session::getSession()->getData('kh_zizhuxiadan')==1){
						$xhtml .= "<a style='cursor:hand;' href='?do=ec_order_order'\">".Pft_I18n::trans('快速下单')."</a>";	
					}else{
						$xhtml .= "<a style='cursor:hand;' href='?do=ec_dingdan_webadd'\">".Pft_I18n::trans('快速下单')."</a>";	
					}
				}
				$xhtml .= '</div>';
			}
			...
			$menuitems = Pft_View_Helper_Js::makeJsArrayExpress( $menu_arr, 'MENU_ITEMS');
