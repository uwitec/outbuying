 ...
 * 在action中这样使用
 * <code>
 * 		$searchs = new Pft_Util_Grid_Searchs( "TpmYonghuPeer" );
...
class Pft_Util_Grid_Searchs{
	 ...
	 * 要求：
	 * Pft_Util_Grid_Searchs::SQL_WHERE_STANDER 前要有一个条件，如 1=1，因为 Pft_Util_Grid_Searchs::SQL_WHERE_STANDER 生成的条件带了一个 and
	 ...
	 * where 1=1 and (".Pft_Config::getCond('a').") ".Pft_Util_Grid_Searchs::SQL_WHERE_STANDER;
	 * //$sql = "select qx_mingcheng, count(qx_yemian) as qx_mingcheng_count from tpm_quanxian as a where 1=1 ".Pft_Util_Grid_Searchs::SQL_WHERE_STANDER." group by qx_mingcheng";
	 * $grid = new Pft_Util_Grid_Searchs();
	 * $grid->setSql( $sql );
	 * $grid->addSearch( 'a.qx_mingcheng',Pft_I18n::trans('qx_mingcheng') );
	 * //$grid->addSearch( 'a.qx_yemian',Pft_I18n::trans('qx_yemian') );
	 ...
	 * 
	 *
	 * @param string|Pft_Util_Grid_Search $sarchObjOrDbColName
	 * @param string $alias
...
//						}
	...
	public function autoGetRequest(){
		 ...
		 * 获得分页信息
		 */
		if( $this->_usePager && ($this->_omPeerName || $this->_sql) && $this->_pager instanceof Pft_Util_Pager ){
			//获得分页信息
			...
			//这里应自动获取页码
			//自动获取页码 已经在pager里了
			//$pager->setPageNum( $this->getInputParameter( Pft_Util_Pager::PAGER_VAR_PAGE_NUM ) );
			//$pager->setPageSize( 3 );
			//if( isset( $_REQUEST[Pft_Util_Pager::PAGER_VAR_PAGE_NUM] ) ){
			//	$this->_pager->setPageNum( $_REQUEST[Pft_Util_Pager::PAGER_VAR_PAGE_NUM] );
			//}
...
class Pft_Util_Grid_Search{
	...
	function __construct( $dbColName = null
                        , $value     = null
                        , $type      = Pft_Util_Grid_Searchs::LIKE
                        , $alias     = null
                        , $rels      = null
                        , $isOr      = null
                        ...
                        )
	{
		...
		$this->_value     = $value;
		$this->_oprate      = $type?$type:Pft_Util_Grid_Searchs::LIKE;
		//$this->_alias     = $alias?$alias:Pft_I18n::trans($dbColName);
		$this->setAlias( $alias );
		$this->_rels      = $rels;
		$this->_isOr      = $isOr;
		...
		if( !( is_null( $this->_value ) || $this->_value === '' ) ){
			...
			switch ( $this->_showType ){
				...
				default:
					if( !is_array( $v ) ){
						//对like 进行一些处理
						$v =( $this->_oprate == Pft_Util_Grid_Searchs::LIKE && strpos( $v, "%" ) === false )?"%".$v."%":$v;						
					}
			}
