...
class ToolsGetCNLine
{
	...
	public function Route($filename)
	{
		...
		while(!feof($fp))
		{
			...
			$match = array();
			$Ret = preg_match_all("/i18ntrans\\([\\'|\\\"]([^'\"]+)|Watt_I18n::trans\\([\\'|\\\"]([^'\"]+)/", $NewLine, $match);
			if($Ret <= 0)
			{
				continue;
			}
			
			//echo "<pre>Terry at [".__FILE__."(line:".__LINE__.")]\nWhen [Wed Aug 25 09:29:04 CST 2010] :\n ";
			//var_dump( $match );
			//echo "</pre>";
			//exit();
			
			for ($i=1;$i<count($match);$i++){
				foreach ($match[$i] as $v) {
					if(trim($v)){
						$this->_strings[$v] = $v;
					}
				}
			}
//			echo "<pre>Terry at [".__FILE__."(line:".__LINE__.")]\nWhen [Wed Aug 25 09:41:27 CST 2010] :\n ";
//			var_dump( $strings );
//			echo "</pre>";
			//exit();
	...
	public function IsSkip($filename)
	{
		...
		if( count($this->_allowList) > 0 ){
			$Result = true;
			foreach($this->_allowList as $Item){
