...
class Watt_View_Helper_Grid
{
	...
	public static function buildGridToOutArray( $grid, $params="", $searchCols=4 )
	{
		...
		if( isset( $grid[ Watt_Util_Grid::GRID_SCHEMA_COLS ] )
		 && count( $grid[ Watt_Util_Grid::GRID_SCHEMA_COLS ] ) > 0 )
		{
			$isDefCols = true;
		...
		if( ( isset( $grid[Watt_Util_Grid::GRID_SCHEMA_SEARCHS] ) && is_array( $grid[Watt_Util_Grid::GRID_SCHEMA_SEARCHS] ) )
		  ||( isset( $grid[Watt_Util_Grid::GRID_SCHEMA_PAGER] ) && is_array( $grid[Watt_Util_Grid::GRID_SCHEMA_PAGER] ) )
		  ||( isset( $grid[Watt_Util_Grid::GRID_SCHEMA_ORDERBYS] ) && is_array( $grid[Watt_Util_Grid::GRID_SCHEMA_ORDERBYS] ) )
		  )
		{						
			...
			*/
			...
			if(isset($grid[Watt_Util_Grid::GRID_SCHEMA_SEARCHS][Watt_Util_Grid_Searchs::DEF_SEARCHGROUP])){
			$searchgroup = $grid[Watt_Util_Grid::GRID_SCHEMA_SEARCHS][Watt_Util_Grid_Searchs::DEF_SEARCHGROUP];	
			}
			if(isset($grid[Watt_Util_Grid_Searchs::DEF_EXPORT])){
				$isexport = $grid[Watt_Util_Grid_Searchs::DEF_EXPORT];	
			}	
			if(isset($grid[Watt_Util_Grid_Searchs::DEF_EXPORTFILE])){
				$exportfile = $grid[Watt_Util_Grid_Searchs::DEF_EXPORTFILE];	
			}	
			if(isset($grid[Watt_Util_Grid_Searchs::DEF_EXPORTFILE_NAME])){
				$exportfile_name = $grid[Watt_Util_Grid_Searchs::DEF_EXPORTFILE_NAME];	
			}
			if(isset($grid[Watt_Util_Grid_Searchs::DEF_EXPORTFILE_FORMAT])){
				$exportfile_format = $grid[Watt_Util_Grid_Searchs::DEF_EXPORTFILE_FORMAT];	
			}
			$exportfilestr = '';
			if(isset($exportfile) && $exportfile){
				$exportfilestr = "&nbsp;<font><a href=".$exportfile.">".Watt_I18n::trans('JT_DINGDAN_XIAZAIWENJIAN')."</a></font>";
			}
			...
			//搜索条件			
			if(isset($searchgroup) && count($searchgroup) && !key_exists( Watt_Util_Grid_Searchs::SEARCH_ADV_SIGN, $SCHEMA_SEARCHS )){//按分组搜索
				$group = $searchgroup;
				//$itemdata = array();
				//组关系
				$groupgx = array();
				...
				foreach ( $searchs as $xb =>$search ){		
					...
					$contstr = '';
					//搜索项
					if( is_array( $search[Watt_Util_Grid_Search::DEF_ITEM] ) && count($search[Watt_Util_Grid_Search::DEF_ITEM]))
					{
						//and or
						$isviewandor=$search[Watt_Util_Grid_Search::DEF_ISVIEWANDOR];
						if($isviewandor && $isviewandor == 'Y'){
							$contstr .= '<select name="isor_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="isor_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" style="width:50px;">';
							if($search[Watt_Util_Grid_Search::DEF_ISOR]){
								$contstr .= '<option value="0">'.Watt_I18n::trans('并且').'</option>';
						...
						$contstr .= '<select name="item_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="item_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'">';
						foreach ( $search[Watt_Util_Grid_Search::DEF_ITEM] as $key=>$value )
						{
							$selSign = ( ($value == $search[Watt_Util_Grid_Search::DEF_TITLE])&&( $search[Watt_Util_Grid_Search::DEF_TITLE] !== '' ) && !is_null( $search[Watt_Util_Grid_Search::DEF_TITLE] ) )?"selected":"";
							$contstr .= '<option value="'.htmlspecialchars($key).'" '.$selSign.'>'.htmlspecialchars($value).'</option>';
						}
						$contstr .= '';
						$contstr .= '</select>';
					}else{
						$contstr .= '<span class="search_name" title="'.$oprationTip.'">'.$search[Watt_Util_Grid_Search::DEF_TITLE].': </span>';
					}
					//$contstr .= '</td><td style="text-align:left">';
					
					//搜索条件设置
					if( is_array( $search[Watt_Util_Grid_Search::DEF_COND] ) && count($search[Watt_Util_Grid_Search::DEF_COND]))
					{
						//这里用下拉列表显示
						$contstr .= '<select name="cond_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="cond_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'">';
						foreach ( $search[Watt_Util_Grid_Search::DEF_COND] as $key=>$value )
						{
							
							$selSign = ( ($value == $search[Watt_Util_Grid_Search::DEF_OPERATION])&&( $search[Watt_Util_Grid_Search::DEF_OPERATION] !== '' ) && !is_null( $search[Watt_Util_Grid_Search::DEF_OPERATION] ) )?"selected":"";
					...
					{
						//这里用下拉列表显示
						$contstr .= '<select class="search_input" name="'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'">';
					...
					else
					{
						switch ( $search[Watt_Util_Grid_Search::DEF_SHOWTYPE] ){
							case Watt_Util_Grid_Search::SHOW_TYPE_DATE;
								...
								$contstr .=  $dateselector->build($search[Watt_Util_Grid_Search::DEF_COLNAME], $search[Watt_Util_Grid_Search::DEF_VALUE], array('class' => 'dateselector') );
								break;
							...
							case Watt_Util_Grid_Search::SHOW_TYPE_SELECTOR_PERSON;
								...
								$contstr .=  $dateselector->build($search[Watt_Util_Grid_Search::DEF_COLNAME], $search[Watt_Util_Grid_Search::DEF_VALUE], array('class' => 'personselector') );
								break;
							case Watt_Util_Grid_Search::SHOW_TYPE_SELECTOR_DINGDAN;
								$dateselector = new Watt_View_Helper_DingdanSelector();
								$contstr .=  $dateselector->build($search[Watt_Util_Grid_Search::DEF_COLNAME], $search[Watt_Util_Grid_Search::DEF_VALUE], array('class' => 'dingdanselector') );
								break;
							case Watt_Util_Grid_Search::SHOW_TYPE_SEARCHTIP;
								
							default:
								$contstr .= '<input style="border:1px solid #000;width:100px;height:19px;" name="'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" value="'.h($search[Watt_Util_Grid_Search::DEF_VALUE]).'">';	
						}
					}
					//$contstr .= '</td>';
					//$output_searchs .= '</div>';
				...
				if( count( $searchs ) ){
					...
					if(isset($isexport) && $isexport){
						$exportstr ='&nbsp;<input type="submit" id="searchFormExport"  name="searchFormExport" value="'.Watt_I18n::trans("ec_dd_daochubaobiao").'" class="btn">';
					...
					<input type="submit" id="searchFormSubmit" value="'.Watt_I18n::trans("SEARCH").'" class="btn">'.$exportstr.$exportfilestr.'
					</div>
					</td>'."\n";	
				}
			}else if (!key_exists( Watt_Util_Grid_Searchs::SEARCH_ADV_SIGN, $SCHEMA_SEARCHS )){//普通搜索
				...
				{									
					...
					if($searchsCounter && $search['isviewandor']){
						$output_searchs .= '<select name="isor_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="isor_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'">';
						if($search[Watt_Util_Grid_Search::DEF_ISOR]){
							$output_searchs .= '<option value="0">'.Watt_I18n::trans('并且').'</option>';
					...
					if( is_array( $search[Watt_Util_Grid_Search::DEF_ITEM] ) && count($search[Watt_Util_Grid_Search::DEF_ITEM]))
					{
						...
						$output_searchs .= '<select name="item_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="item_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'">';
						foreach ( $search[Watt_Util_Grid_Search::DEF_ITEM] as $key=>$value )
						{
							$selSign = ( ($value == $search[Watt_Util_Grid_Search::DEF_TITLE])&&( $search[Watt_Util_Grid_Search::DEF_TITLE] !== '' ) && !is_null( $search[Watt_Util_Grid_Search::DEF_TITLE] ) )?"selected":"";
					...
					//搜索条件设置
					//Watt_Util_Grid_Search::DEF_OPERATIONVIEW;		
					//$search['operationview']=array('等于'=>'=','小于'=>'<','包含'=>Watt_Util_Grid_Searchs::LIKE);
					if( is_array( $search[Watt_Util_Grid_Search::DEF_COND] ) && count($search[Watt_Util_Grid_Search::DEF_COND]))
					{
						//这里用下拉列表显示
						$output_searchs .= '<select name="cond_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'" id="cond_'.$search[Watt_Util_Grid_Search::DEF_COLNAME].'">';
						foreach ( $search[Watt_Util_Grid_Search::DEF_COND] as $key=>$value )
						{
...
//				}
				...
				if( count( $searchs ) ){
					...
					$exportstr ='';
					if(isset($isexport) && $isexport){
						$exportstr ='&nbsp;<input type="button" id="searchFormExport1" onmouseover="document.getElementById(\'exportDiv\').style.display=\'\';" onclick="document.getElementById(\'exportDiv\').style.display=\'\';"  name="searchFormExport1" value="'.Watt_I18n::trans("ec_dd_daochubaobiao").'" class="btn">';
					...
					if($_REQUEST['do']=='res_yiyuanwanquan_zongku'){
						$zongkusearchall_but= "&nbsp;<input type='button' value='".i18ntrans('#全字段检索')."' onclick=\"window.location='?do=res_yiyuanwanquan_zongkusearchall'\" class='btn' />";
					}
					//如果没有修改搜索条件，不会影响总记录的条数<tr>
					$output_searchs .= '<td colspan="'.($number*4).'">
					<div style="clear:both;text-align:center;border:1px">
					<input type="submit" id="searchFormSubmit" value="'.Watt_I18n::trans("SEARCH").'" class="btn">'.$exportstr.$exportfilestr.$zongkusearchall_but.'
					</div>
					</td>'."\n";	
				}
			}else{
			 ...
			 * 如果有排序定义，输出排序表单域
			 */
			if( is_array( $grid[Watt_Util_Grid::GRID_SCHEMA_ORDERBYS] )
			 && count( $grid[Watt_Util_Grid::GRID_SCHEMA_ORDERBYS][Watt_Util_Grid_Searchs::DEF_ORDERBYS] ) )
			{
				...
				$orderByCols[$grid[Watt_Util_Grid::GRID_SCHEMA_ORDERBYS][Watt_Util_Grid_Searchs::DEF_ORDERBYS][0]] 
				      = $grid[Watt_Util_Grid::GRID_SCHEMA_ORDERBYS][Watt_Util_Grid_Searchs::DEF_ORDERBYORDERS][0]==Watt_Util_Grid_Searchs::DESC?"↓":"↑";
			}
			else
			{
...
//EOT;
		...
		if(isset($_REQUEST['searchFormExport']) && $_REQUEST['searchFormExport']){//导出报表
			...
			$exportCol = $grid[Watt_Util_Grid::GRID_SCHEMA_EXPORT_COL];
			//报头设置
			if( $isDefCols )
			{
				...
				foreach ( $cols as $col )
				{
					$biaotou[] = self::clearHtml( ( is_null( $col["title"] ) )?Watt_I18n::trans($col["colname"]):$col["title"] );
				}
			}
			else
			{
			...
			//导出报表
			if($exportfile_name){
				//$file=Watt_Util_Export::ExporttoCsv($baobiaodata,$exportfile_name,true);
				...
				{
					$file=Watt_Util_Export::ExportToXls($baobiaodata,$exportfile_name,true);
				}
			}else{
				//$file=Watt_Util_Export::ExporttoCsv($baobiaodata,"Export");
				if($exportfile_format=='csv')
				{
					$file=Watt_Util_Export::ExportToCsv($baobiaodata,"Export");
				}
				...
				{
					$file=Watt_Util_Export::ExportToXls($baobiaodata,"Export");
				}
			}
			if($file){
				$exportfilestr = '';
				//if(isset($exportfile) && $exportfile){
					$exportfilestr = "<div style='text-align:center;' class='notice'>&nbsp;<font><a href='".$file."' class='btn'>".Watt_I18n::trans('JT_DINGDAN_XIAZAIWENJIAN')."</a></font>&nbsp;<a href='#' onclick='window.history.go(-1);return false;' class='btn'>".Watt_I18n::trans('返回')."</a></div>";
				//}
				//$this->_exportfile = $file;
			}
			$rev[Watt_Util_Grid::GRID_SCHEMA_DATAS] = $exportfilestr;		
			return $rev;
		}
		/**
		 * Body 信息
...
//			}
		...
		if( is_array( $grid[Watt_Util_Grid::GRID_SCHEMA_PAGER] ) && $grid[Watt_Util_Grid::GRID_SCHEMA_PAGER][Watt_Util_Pager::PAGER_VAR_PAGE_COUNT] > 0 ){	// 只要存在数据就显示 GRID FOOTER，具体显示哪些元素由 toHtml 函数内判断 //bobit Tue Dec 11 10:13:54 CST 200710:13:54
			...
			if( $cols ){
				foreach ( $datas as $rowKey => $row ){
					...
					foreach ( $cols as $colKey => $col ){
						 ...
						 */
						if( $rowKey === Watt_Util_Grid_Searchs::DEF_SUM || $rowKey === Watt_Util_Grid_Searchs::DEF_TOTAL ){
							//注意，这里要用 === ，否则第一行会出错
							if( $colNum == 0 ){
								$showText = '<b>'.Watt_I18n::trans($rowKey).'</b>';
							}else{
