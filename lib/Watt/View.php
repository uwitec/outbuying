...
abstract class Watt_View{
	...
	{
		$className = "Watt_View_".ucfirst( $classNameLast );
 	...
 	function __get($nm)
	{
		...
		{
            $e = new Watt_Exception(Watt_I18n::trans("ERR_VIEW_NODATA"));
            throw($e);
            return "";
        }
 	}
 	...
 	/**
 	 * 将一些需要显示的数据合并到view中
 	 * 要求数据是 在 Watt:Data 根下的数据
 	 *
 	 * @param array $data Watt_Data_Schema
 	 */
