...
//		$this->_gridData[self::GRID_SCHEMA_PAGER]    = array();
		...
		if( $data instanceof Watt_Util_Grid ){
	 ...
	 * 
	 * <code>
	 * $grid = new Watt_Util_Grid( $list_data );
	 * $grid->addCol( Watt_I18n::trans("email"),"email", true, '"<a href=\"mailto:".$row["email"]."\">".$row["email"]."</a>"' );
	 * </code>
	 *
	 * @param string $title
...
//		}
	...
	{
		$out = Watt_View_Helper_Grid::buildGrid( $this->getGridData(), false, $formAttribs );		
		echo $out;
	}
	 ...
	 * 
	 * 包括:
	 * $rev[Watt_Util_Grid::GRID_SCHEMA_SEARCHS]
	 * $rev[Watt_Util_Grid::GRID_SCHEMA_DATAS]
	 * $rev[Watt_Util_Grid::GRID_SCHEMA_PAGER]
	 *
	 * @return array
	 */
	public function getOutputArray()
	{
		return Watt_View_Helper_Grid::buildGridToOutArray( $this->getGridData() );
	}
	...
	public function getRenderedArray(){
		...
		return Watt_View_Helper_Grid::buildGridToDataArray( $this->getGridData() );
	}
	...
	{
		return Watt_View_Helper_Grid::buildGrid( $this->getGridData(), false );
	}
	 ...
	 * 
	 *
	 * @return Watt_Util_Searchs
	 */
	private function _getSearchs(){
		if( !$this->_searchs ){
			$this->_searchs = new Watt_Util_Searchs();
		}
