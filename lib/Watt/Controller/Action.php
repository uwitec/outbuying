 ...
 * @author Terry
 * @version 0.1.3
 * Fri May 23 10:54:25 CST 2008	增加权限的验证
 * Mon Nov 10 11:10:02 CST 2008 增加将一个权限映射到多个权限上的处理
 * @package Watt_Controller
 */
class Watt_Controller_Action{
	...
	//private $_ctrlLevel = Watt_Rbac::LEVEL_ROLE;	//默认权限不能太低 by terry at Fri Jul 10 11:30:32 CST 2009
	private $_ctrlLevel = Watt_Rbac::LEVEL_LOGIN;
	 ...
	 * @var array
	 */
	private $_publicActionsList = array("index" => Watt_Rbac::LEVEL_PUBLIC
	                                     );
	/**
	 * 权限映射
	 * 
    ...
    protected function __get($varname)
	{
		...
		else
		{
			...
			/*
    		//return "";
    		$e = new Watt_Exception(Watt_I18n::trans("ERR_CLASS_NO_PROPERTY"));
    		throw $e;
    		*/
    	}
    }
	 ...
	 * @return Watt_Controller_Action
	 */
	//public static final function factory($controllerName, $actionName){
	public static final function factory($controllerAndActionName){
		//$toFile = Watt_Config::getAppPath() . $controllerAndActionName . ".php";
		$toFile = Watt_Config::getAbsPathFilename( "PATH_APP", $controllerAndActionName . ".php" );
		$arrTmp = array_map("ucfirst", explode( DIRECTORY_SEPARATOR, $controllerAndActionName) );
...
通过上面的代码改成一个controller多个action的形式
		...
		//这种形式要求 conntroller
		$toFile = PATH_APP . $controllerAndActionName . DIRECTORY_SEPARATOR . $actionName . ".php";
...
*/
		...
		//Watt::loadFile( $toFile );
		if( class_exists( $className ) ){
			$class = new $className();
			$class->setScriptFile($toFile);
			//$class->_autoGetControllerName();
			$class->setControllerName( str_replace( DIRECTORY_SEPARATOR, "_", $controllerAndActionName ) );
			//自动获取名称不够准确，只能继续使用第二种方法 by terry at Mon Jul 13 11:03:22 CST 2009
			$class->setControllerPrivilegeMaps();
			return $class;
		}else{
			throw (new Exception(Watt_I18n::trans("ERR_APP_LOST_CONTROLLER") . "[ ".$className." ]" ));
			//return null;
		}
	}
     ...
     * 
     * <code>
     * $this->addTip( Watt_I18n::trans("MSG_SEND_SUCCESS"), "?do=".$this->getSelfActionDo("list") );
     * </code>
     *
     * @param string $actionName
     * @return string
	...
	*/
	...
	protected function setPublic( $v ){
		//$this->_isPublic = $v;
		if( $v ){
			$this->_ctrlLevel = Watt_Rbac::LEVEL_PUBLIC;
		}
	}
	...
	public function isPublic(){
		...
		return ( Watt_Rbac::LEVEL_PUBLIC == $this->_ctrlLevel );
	}
	
	/**
	 * @param Watt_Rbac::LEVEL_* $v
	 */
	...
	public function isActionPublic( $actionName ){
		if( key_exists( $actionName, $this->_publicActionsList ) 
		 && $this->_publicActionsList[$actionName] == Watt_Rbac::LEVEL_PUBLIC )
		{
			return true;
		}
		return false;
