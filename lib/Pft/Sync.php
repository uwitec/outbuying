...
class Pft_Sync
{
	...
	public static function InitiativeInput()
	{
		...
		{//同步模式（SyncMode）不为1时执行					
				...
				foreach ($msgSql as $k => $v){
					...
					if($re==$k){
						...
						$data = array('msgSign' => urlencode(serialize($k)),
										'validatePassword'=>Pft_Sync_MessageListManage::getValidatePassword(),
										'msgType' =>'msgSql'
									);
			...
			$num_del = Pft_Sync_MessageListManage::getNumOnceDel();//一次删除消息序列的个数
			//消息序列的处理			
	...
	{		
		...
		//判断是否锁定
		if (Pft_Sync_MessageListManage::isLock('output'))
		{//如果锁定
			exit;
		}
		//锁定接口
		Pft_Sync_MessageListManage::AddLock('output');
		//调用消息序列管理器，获取消息序列
		/*
		$msg = new MSG();
		$msg->openMsgPool();
		...
		*/
		//获取所有的SQL消息
		$msgSqlArray = Pft_Sync_MessageListManage::getAllMessageSql();
		$msgSqlValue = array();
		
		if(is_array($msgSqlArray) && count($msgSqlArray)){
			foreach ($msgSqlArray as $k => $v){
				$rev = Pft_Sync_MessageListManage::executeMessageSql($v);
				if ($rev){
					//获取外部被动输入接口地址
					$passiveInputAddress = Pft_Sync_MessageListManage::getPassiveInputInterface('INSIDE_TPM');					
					...
					{//获取地址成功
						...
						{//成功
							//删除消息SQL
							Pft_Sync_MessageListManage::delMessageSql($k);			
						}else{
							//移动消息SQL
							Pft_Sync_MessageListManage::moveMsgSql($k);
						}
					}
				}
			}
		...
		if($passiveOutputAddress){
			...
			{//unknown目录中存在数据
				...
				//删除本地的消息
				//$result = Pft_Sync_MessageListManage::delMessageList($msgSignArr)
				$result = Pft_Sync_MessageListManage::moveMsglistOld($msgSignArr);
				if ($result)
				{//删除成功
					...
					if ($rev1){
						...
						//解除锁定
						Pft_Sync_MessageListManage::DelLock('output');
						exit;
					}
				}else{
		...
		if (is_array($msgListArray) && count($msgListArray) && $passiveInputAddress){
			...
			{				
				foreach ($val as $k =>$v){
					//--------------------- 数据过滤器  start---------------------//
					//根据创建消息序列的服务器类型获取过滤器
					$filterNameList = Pft_Sync_MessageListManage::getMsgListFilter($v['syncServerType']);				
					//调用过滤器过滤数据				
				...
				{//满足个数
					...
					if ($rev == '1'){
						//删除
						if (Pft_Sync_MessageListManage::delMessageList($msgSignArr))
						{//删除成功
							//以post的方式向接口提交数据
							$data = array(
											'obj' =>'file',
	...
	{	
		//访问权限判断
		if(!Pft_Sync_MessageListManage::interfaceAuth('PassiveInput'))return false;
		//验证密码
		if(!Pft_Sync_MessageListManage::passwordAuth($pwd))return false;
	...
	public static function getDbInfo($pwd='')
	{
		//验证密码
		if(!Pft_Sync_MessageListManage::passwordAuth($pwd))return false;
		return Pft_Sync_MessageListManage::getmsg1();
	}			
	/**
	 * 获取数据库所有表的记录数
	 * 功能：获取本地的数据库信息
	...
	public static function getDbAllTableCount($pwd='')
	{
		//验证密码
		if(!Pft_Sync_MessageListManage::passwordAuth($pwd))return false;		
		$re = Pft_Sync_MessageListManage::getDbAllTableCount();
